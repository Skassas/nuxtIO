export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await pb.collection('fm_folders').update(id, { name: cleanName })
    return result
  } catch (err: any) {
    console.error('[fm_folders/update] PB hata:', err?.message || err)
    throw createError({
      statusCode: 500,
      data: { error: 'SERVER_ERROR', message: err?.message || 'Klasör güncellenirken hata oluştu' },
    })
  }
})
