...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const result = await pb.collection('taxes').getList(page, perPage, {
      sort,
      filter,
    })
    return result
  } catch (err: any) {
    console.error('[taxes/index.get] PB hata:', err?.message || err)
    throw createError({
      statusCode: 500,
      data: { error: 'SERVER_ERROR', message: 'Vergiler getirilirken bir hata oluştu' },
    })
  }
})
