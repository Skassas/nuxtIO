<template>
  <div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex-1 flex justify-center">
        <div class="relative w-full max-w-md">
          ...
          <input v-model="searchQuery" @keyup.enter="search" type="text" placeholder="Müşteri ara..."
    ...
    <CustomersTable :customers="items" :loading="loading" :current-page="currentPage" :total-pages="totalPages" :per-page="perPage" :total-items="totalItems"
      :sort-by="sortBy" :sort-order="sortOrder"
      @view="openViewDrawerFetch" @edit="openEditDrawer" @delete="handleDelete" @prev-page="currentPage--" @next-page="currentPage++" @sort="setSort" />

    <AdminDrawer :isOpen="drawerOpen" :title="drawerTitle" :icon="drawerIcon" @close="closeDrawer">
      <CustomersView v-if="drawerMode === 'view'" :customer="selectedItem" />
      <CustomersForm v-else v-model="form" :errors="formErrors" :isEdit="drawerMode === 'edit'" @submit="handleSubmit" />
      <template #footer>
...
import CustomersTable from './_CustomersTable.vue'
import CustomersForm from './_CustomersForm.vue'
import CustomersView from './_CustomersView.vue'
import type { Customer } from './_CustomersTable.vue'
...
const {
  ...
  itemName: 'Müşteri',
  icon: CustomerIcon,
  defaultSortBy: 'customer_first_name',
})
